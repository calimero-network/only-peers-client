...
export class AxiosHttpClient implements HttpClient {
    ...
    async get<T>(url: string, headers?: Header[]): Promise<ResponseData<T>> {
        return this.request<T>(this.axios.get<ResponseData<T>>(url, {
            headers: headers?.reduce((acc, curr) => ({...acc, ...curr}), {})
        }));
    }

    async post<T>(url: string, body?: unknown, headers?: Header[]): Promise<ResponseData<T>> {
        return this.request<T>(this.axios.post<ResponseData<T>>(url, body, {
            headers: headers?.reduce((acc, curr) => ({...acc, ...curr}), {})
        }));
    }

    async put<T>(url: string, body?: unknown, headers?: Header[]): Promise<ResponseData<T>> {
        return this.request<T>(this.axios.put<ResponseData<T>>(url, body, {
            headers: headers?.reduce((acc, curr) => ({...acc, ...curr}), {})
        }));
    }

    async delete<T>(url: string, headers?: Header[]): Promise<ResponseData<T>> {
        return this.request<T>(this.axios.delete<ResponseData<T>>(url, {
            headers: headers?.reduce((acc, curr) => ({...acc, ...curr}), {})
        }));
    }

    async patch<T>(url: string, body?: unknown, headers?: Header[]): Promise<ResponseData<T>> {
        return this.request<T>(this.axios.patch<ResponseData<T>>(url, body, {
            headers: headers?.reduce((acc, curr) => ({...acc, ...curr}), {})
        }));
    }
    ...
    private async request<T>(promise: Promise<AxiosResponse<ResponseData<T>>>): Promise<ResponseData<T>> {
        try {
